...
class AIEngine:
    ...
    def get_models_dir(self, force_writable=False):
        ...
        - Se force_writable è True, ritorna SPRERE la cartella in AppData (per i download).
        - Altrimenti, cerca prima localmente (Program Files) e poi in AppData.
        """
        import platform
        system = platform.system()
        
        # 0. Definiamo i due percorsi possibili
        # Percorso Utente (Sempre Scrivibile)
        if system == "Windows":
            user_base = os.path.join(os.environ.get("LOCALAPPDATA", os.path.expanduser("~")), "Datarium")
        elif system == "Darwin":
            user_base = os.path.expanduser("~/Library/Application Support/Datarium")
        else:
            user_base = os.path.expanduser("~/.datarium")
        user_models_dir = os.path.join(user_base, "models")

        if force_writable:
            if not os.path.exists(user_models_dir): os.makedirs(user_models_dir, exist_ok=True)
            return user_models_dir

        # Percorso Locale (Sola Lettura se in Program Files)
        if getattr(sys, 'frozen', False):
            if hasattr(sys, '_MEIPASS'):
                local_models_dir = os.path.join(sys._MEIPASS, "models")
            else:
        ...
        # 1. Se i modelli esistono nel percorso locale, usiamo quello (priorità installer)
        if os.path.exists(local_models_dir):
            if os.path.exists(os.path.join(local_models_dir, self.text_file)) or \
               os.path.exists(os.path.join(local_models_dir, "phi-2.Q2_K.gguf")):
                return local_models_dir

        # 2. Altrimenti usiamo il percorso utente (già esistente o per futuri download)
        return user_models_dir

    def check_models_missing(self):
        """Controlla se i modelli esistono in una QUALSIASI delle cartelle supportate."""
        try:
            import platform
            system = platform.system()
            ...
            # Path Locale
            if getattr(sys, 'frozen', False):
                check_dirs.append(os.path.join(os.path.dirname(sys.executable), "models"))
            else:
                check_dirs.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), "models"))
                
            # Path Utente
            if system == "Windows":
                check_dirs.append(os.path.join(os.environ.get("LOCALAPPDATA", ""), "Datarium", "models"))
            elif system == "Darwin":
                check_dirs.append(os.path.expanduser("~/Library/Application Support/Datarium/models"))
            ...
            return True # Qualcosa manca ovunque
        except:
            return True
    ...
    def download_model_if_needed(self, vision_mode=True, progress_callback=None):
        ...
        try:
            ...
            for repo, filename in tasks:
                ...
                # Verifichiamo se il file esiste già in QUALSIASI cartella (Local o AppData)
                # Prima usiamo get_models_dir(force_writable=False) per cercare ovunque
                current_dir = self.get_models_dir(force_writable=False)
                path = os.path.join(current_dir, filename)
                alt_path = os.path.join(current_dir, alt_filename)

                if not os.path.exists(path) and not os.path.exists(alt_path):
                    # Se manca, dobbiamo SCARICARE. 
                    # Forza SPRE il percorso AppData per EVITARE Accesso Negato in Program Files
